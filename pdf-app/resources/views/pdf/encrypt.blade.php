<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-700 leading-tigh flex items-center justify-center gap-2">
                ...
                <form id="encrypt-form" enctype="multipart/form-data">
                    ...
                    <div class="relative mt-4">
                        ...
                        <input
                            ...
                            name="password"
                            id="password"
                            placeholder="{{__('encrypt.password')}}"
                ...
                <div id="result" class="mt-6 hidden">
                    <div class="flex items-center bg-white border border-gray-300 rounded-md px-4 py-3 shadow-sm justify-center">
                        ...
                        <span class="text-sm text-gray-800">
                            {{__('encrypt.encrypted')}} <a id="download-link" href="#" download="encrypted.pdf"
                                class="text-amber-600 font-medium underline ml-1" target="_blank">{{__('downloadPDF')}}</a>
                        </span>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        document.getElementById('encrypt-form').addEventListener('submit', async function(e) {
            ...
            const response = await fetch("{{ route('pdf.encrypt.upload') }}", {
                method: "POST",
                headers: {
        ...
        function showModal(message) {
            alert(message); // Replace with modal if desired
        }
    </script>
</x-app-layout>
